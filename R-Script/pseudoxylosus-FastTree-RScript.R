...
df <- read_excel("pseudoxylosus-Metadata-FastTree.xlsx")
...
tree <- ape::read.tree("pseudoxylosus-coregenome-FastTree.newick")
...
ggtree(tree) %<+% df +  
  ...
  scale_fill_manual(values = c("Fly" = "#cd3600", 
                               ...
                               "Other" = "#c3c7c3" )) + 
  theme(legend.position="none") +
  expand_limits(x = max(tree$edge.length) + 0.02)
