...
df <- read_excel("Xylosus-Metadata-FastTree.xlsx")
...
tree <- ape::read.tree("xylosus-coregenome-FastTree.newick")
...
p1<- ggtree(tree) %<+% df +  
  ...
  scale_fill_manual(values = c("Fly" = "#cd3600", 
                               ...
                               "Arthropod" = "#c3c7c3",
                               "Other" = "#c3c7c3" )) + 
  theme(legend.position="none") +   geom_tiplab(size=1.5, hjust =-0.2) 
...
p2<- ggtree(tree, branch.length='none', layout='circular') %<+% df +  
  geom_tippoint(aes(fill = Origin), size = 2, shape=22) +
  scale_fill_manual(values = c("Fly" = "#cd3600", 
                               "Manure" = "#648fff", 
                               "Cattle" = "#b7dbeb", 
